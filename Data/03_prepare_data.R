...
data2.1 <- data1.1 %>% 
 select(ICO, year, ebit, ebitda, financial.expanses, net.worth, total.debt, AKTIVACELK) %>% 
...
data6.1 <- data5 
data6.1$r3t1 <- c(diff(data6.1$net.worth), 0)
data6.1$rule3 <- ifelse(data6.1$r3t1 <0, 1, 0)
data6.1$rule3 <- ifelse(data6.1$r3t1 <0 & data6.1$last==1, 0, data6.1$rule3)

# Combine rules
data6.2 <- data6.1 %>% select(-r3t1)
data6.2$def <- with(data6.2, ifelse(rule1 + rule2 + rule3==3, 1, 0))
data6.3 <- data6.2 %>% filter(last==0)

#=======================================
 data6 <- data6.3 %>% 
  select(-ebitda, -ebit, -financial.expanses, -net.worth, -total.debt)
  
#---- Class==1 : companies with higher than mode ebitda or ebit / AKTIVACELK
median.all <- data6.3 %>% group_by(ICO) %>% summarise(med = quantile(ebit/AKTIVACELK, probs=0.5)) %>% 
 select(med) %>% summarise(median = quantile(med, probs=0.5)) %>% as.numeric()

add.class <- data6.3 %>% group_by(ICO) %>% 
 summarise(
